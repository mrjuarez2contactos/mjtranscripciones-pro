...
const App: React.FC = () => {
    ...
    const processSingleFile = async (itemId: string) => {
        ...
        try {
            ...
            updateFileInQueue(itemId, { // Corregido el nombre de la función auxiliar
    ...
    // Wrapper para el botón individual
                                ...
                                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem'}}>
                                    ...
                                    <div> 
                                        <button 
                                            ...
                                            disabled={isLoading || fileQueue.filter(f => f.status === 'pending' || f.status === 'error').length === 0}
                                            style={{...styles.button, ...styles.buttonGreen, margin: 0, ...( (isLoading || fileQueue.filter(f => f.status === 'pending' || f.status === 'error').length === 0) ? styles.buttonDisabled : {})}}
                                        >
