...
    // Leemos directamente desde el estado actual
...
      padding: '10px',
      borderRadius: '6px',
      border: '1px solid #dddfe2', // <-- ¡AQUÍ ESTABA EL ERROR!
      fontSize: '14px',
...
  const getStatusStyle = (st: FileStatus): React.CSSProperties => {
    switch (st) {
      case 'processing':
...
            <button style={{ ...styles.button, margin: 0 }} onClick={() => setIsModalOpen(true)}>
              Mejoras
            </button>
            <button
...
              disabled={isLoading}
            >
              {isLoading && status.startsWith('Actualizando')
                ? 'Cargando...'
                : showAnotaciones
                ? 'Ocultar Resúmenes'
                : 'Ver Resúmenes'}
...
                  </div>
                  <p style={styles.anotacionResumen}>{item.resumen || '(Sin resumen)'}</p>
                </div>
              ))}
            </div>
          </div>
...
        {/* UI PRINCIPAL (OCULTA SI VISOR ESTÁ ACTIVO) */}
        {!showAnotaciones && (
          <>
...
                <label
                  htmlFor="file-upload"
                  style={{ ...styles.button, cursor: 'pointer', flex: 1, textAlign: 'center', minWidth: '200px', margin: 0 }}
                >
                  Subir desde PC
                </label>
                <input
...
                        ...styles.buttonGreen,
                        margin: 0,
                        ...(isLoading || fileQueue.filter(f => f.status === 'pending').length === 0
...
                      {isLoading
                        ? 'Procesando...'
                        : `Procesar Todos (${fileQueue.filter(f => f.status === 'pending').length})`}
                    </button>
                  </div>
                </div>
...
                          }}
                        >
s                         <span style={styles.queueItemName}>{item.displayName}</span>
...
                              ...styles.buttonSmall,
                              margin: 0,
                              ...(isLoading || item.status === 'processing' || item.status === 'completed'
section                         ? styles.buttonDisabled
                                : {}),
                            }}
                          >
...
                              ...styles.buttonGreen,
                              margin: 0,
                      s       ...(item.status !== 'completed' ? styles.buttonDisabled : {}),
                            }}
                          >
                            Descargar
JSON                     </button>
...
                              margin: 0,
                              backgroundColor: '#fa3e3e',
                  S           ...(isLoading || item.status === 'processing' ? styles.buttonDisabled : {}),
s                       }}
                          >
                            Quitar
                          </button>
...
                      s   {item.errorMessage.includes('PROHIBITED_CONTENT') ||
                          item.errorMessage.includes('blocked') ? (
                    s       <span>
                              <strong>Contenido Prohibido:</strong> Google ha bloqueado este audio. Elimine este archivo.
                            </span>
                          ) : (
                            <span>
                          s   <strong>Error:</strong> {item.errorMessage.substring(0, 200)}...
                            </span>
                          )}
                        </div>
Note                     )}
...
s                 style={{ ...styles.button, margin: 0, backgroundColor: '#606770' }}
                >
                  Cancelar
                </button>
Real             <button
...
                  style={{ ...styles.button, flex: 1, backgroundColor: '#42b72a', margin: 0 }}
                >
...
                    }
                  }}
Note               style={{ ...styles.button, ...styles.modalButton, margin: 0 }}
                >
                  Añadir
                </button>
              </div>
...
OS             {globalInstructions.map((inst, index) => (
...
                style={{ ...styles.button, marginTop: '1rem', margin: 0 }}
              >
      s         Cerrar
              </button>
iOS           </div>
...
  if (!response.ok) {
s   const errorData = await response.json();
...
// --- FUNCIONES LEGACY (PARA FLUJO PC) ---
